...
// 专属配置：自动清理 URL，确保连接无误 (已内置您的专属密钥)
...
export default function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [resCab, resDoor, resUpg, resRule] = await Promise.all([
        ...
        supabase.from('upgrade_items').select('*').order('sort_order', { ascending: true }).order('created_at', { ascending: false }),
      ...
      showToast('数据同步异常: ' + error.message, 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleToggleUpgradeStatus = async (item) => {
    try {
      ...
      showToast(item.status ? '已停用该项目' : '已重新启用该项目');
      fetchData();
  ...
  const handleDelete = async (table, id) => {
    if (!window.confirm('危险操作：确定要彻底删除吗？若该材料曾被报价使用，可能会导致历史报表报错。建议仅删除建错的测试数据。')) return;
    try {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center font-sans">
        <div className="bg-white p-10 rounded-2xl shadow-2xl w-full max-w-md border border-gray-100 relative overflow-hidden">
          ...
          <div className="text-center mb-10 mt-4">
            ...
            <p className="text-sm text-gray-500 mt-2 font-medium tracking-widest uppercase">诺一家具内部管理系统</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-2 uppercase tracking-wide">账号 (Username)</label>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-2 uppercase tracking-wide">密码 (Password)</label>
          ...
          <div className="mt-8 text-center text-[10px] text-gray-400 uppercase tracking-widest font-bold">Version 2.3 Core System</div>
        </div>
    ...
    <div className="flex h-screen bg-gray-100 font-sans overflow-hidden">
      ...
      <div className="w-64 bg-gray-900 text-white flex flex-col shadow-2xl z-20">
        ...
        <div className="flex-1 overflow-y-auto py-6">
          <div className="px-6 text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">基础数据台</div>
          ...
          <button onClick={() => {setCurrentView('door'); setEditId(null);}} className={`w-full text-left px-6 py-4 font-medium transition-colors border-l-4 ${currentView==='door'?'border-blue-500 bg-gray-800 text-white':'border-transparent text-gray-400 hover:text-white hover:bg-gray-800/50'}`}>🚪 基础门板库</button>
          <button onClick={() => {setCurrentView('upgrade'); setEditId(null);}} className={`w-full text-left px-6 py-4 font-medium transition-colors border-l-4 ${currentView==='upgrade'?'border-amber-500 bg-gray-800 text-white':'border-transparent text-gray-400 hover:text-white hover:bg-gray-800/50'}`}>✨ V2 升级项目库</button>
          
          <div className="px-6 text-xs font-bold text-gray-500 uppercase tracking-widest mb-3 mt-8">引擎配置</div>
      ...
      <div className="flex-1 overflow-y-auto p-10 relative">
        {isLoading && <div className="absolute top-6 right-10 bg-blue-100 text-blue-800 px-4 py-2 rounded-full text-xs font-bold shadow animate-pulse z-50 flex items-center gap-2"><span className="animate-spin text-lg">↻</span> 数据同步中...</div>}
        ...
        {currentView === 'cabinet' && (
          <div className="max-w-6xl mx-auto space-y-6 animate-fade-in">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
              <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">{editId ? <span className="text-blue-600">📝 编辑柜体材料</span> : '➕ 新增柜体材料'}</h3>
              <form onSubmit={handleSaveCabinet} className="grid grid-cols-5 gap-4 items-end">
                <div className="col-span-2"><label className="block text-xs font-bold text-gray-500 mb-2">材料名称</label><input required value={cabinetForm.name} onChange={e=>setCabinetForm({...cabinetForm, name:e.target.value})} className="w-full border-2 border-gray-200 p-2 rounded-lg focus:border-black focus:outline-none" placeholder="如: 18mm实木多层板" /></div>
                ...
                <div><label className="block text-xs font-bold text-gray-500 mb-2">无门板补偿系数</label><div className="flex gap-2"><input required type="number" step="0.01" value={cabinetForm.no_door_factor} onChange={e=>setCabinetForm({...cabinetForm, no_door_factor:e.target.value})} className="w-full border-2 border-gray-200 p-2 rounded-lg focus:border-black focus:outline-none" placeholder="如: 1.2" /><button type="submit" className="bg-black text-white px-6 rounded-lg font-bold hover:bg-gray-800 whitespace-nowrap shadow">{editId ? '保存' : '添加'}</button></div></div>
              </form>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 text-gray-500 font-bold border-b border-gray-200 text-xs uppercase tracking-wider"><tr className="divide-x divide-gray-100"><th className="p-4">材料名称</th><th className="p-4">标准单价</th><th className="p-4">浅柜单价</th><th className="p-4">无门补偿系数</th><th className="p-4 text-center">操作</th></tr></thead>
                <tbody className="divide-y divide-gray-100">
                  {cabinets.map(item => (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors"><td className="p-4 font-bold text-gray-800 text-base">{item.name}</td><td className="p-4 font-medium">¥{item.base_price}</td><td className="p-4 font-medium text-blue-600">¥{item.shallow_price}</td><td className="p-4 font-bold text-rose-600 bg-rose-50/50">× {item.no_door_factor}</td>
                      <td className="p-4 text-center space-x-4"><button onClick={() => {setEditId(item.id); setCabinetForm(item);}} className="text-blue-600 font-bold hover:underline">编辑</button><button onClick={() => handleDelete('materials_cabinet', item.id)} className="text-gray-400 hover:text-rose-600">删除</button></td>
                    </tr>
                  ))}
                  {cabinets.length === 0 && <tr><td colSpan="5" className="p-8 text-center text-gray-400">暂无数据，请在上方添加</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
              ...
              <form onSubmit={handleSaveDoor} className="grid grid-cols-4 gap-4 items-end">
                <div><label className="block text-xs font-bold text-gray-500 mb-2">门板名称</label><input required value={doorForm.name} onChange={e=>setDoorForm({...doorForm, name:e.target.value})} className="w-full border-2 border-gray-200 p-2 rounded-lg focus:border-black focus:outline-none" placeholder="如: PET高定门板" /></div>
                <div><label className="block text-xs font-bold text-gray-500 mb-2">门板类型 (核心逻辑)</label><select value={doorForm.door_type} onChange={e=>setDoorForm({...doorForm, door_type:e.target.value})} className="w-full border-2 border-gray-200 p-2 rounded-lg bg-white focus:border-black focus:outline-none"><option value="普通门板">普通门板 (参与常规计价)</option><option value="特殊门板">特殊门板</option><option value="无门板">无门板 (触发无门补偿系数)</option></select></div>
                <div><label className="block text-xs font-bold text-gray-500 mb-2">基础单价 (元/㎡)</label><input required type="number" disabled={doorForm.door_type === '无门板'} value={doorForm.door_type === '无门板' ? 0 : doorForm.base_price} onChange={e=>setDoorForm({...doorForm, base_price:e.target.value})} className="w-full border-2 border-gray-200 p-2 rounded-lg focus:border-black focus:outline-none font-bold text-gray-800 disabled:bg-gray-100 disabled:text-gray-400" /></div>
                <div><button type="submit" className="w-full bg-black text-white p-2 rounded-lg font-bold hover:bg-gray-800 h-11 shadow">{editId ? '保存修改' : '确认添加'}</button></div>
              </form>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 text-gray-500 font-bold border-b border-gray-200 text-xs uppercase tracking-wider"><tr className="divide-x divide-gray-100"><th className="p-4">门板名称</th><th className="p-4">系统类型</th><th className="p-4">基础单价</th><th className="p-4 text-center">操作</th></tr></thead>
                <tbody className="divide-y divide-gray-100">
                  {doors.map(item => (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors"><td className="p-4 font-bold text-gray-800 text-base">{item.name}</td><td className="p-4"><span className={`px-2 py-1 rounded text-xs font-bold ${item.door_type==='无门板'?'bg-rose-100 text-rose-800':'bg-emerald-100 text-emerald-800'}`}>{item.door_type}</span></td><td className="p-4 font-medium">{item.door_type === '无门板' ? '-' : `¥${item.base_price}`}</td>
                      <td className="p-4 text-center space-x-4"><button onClick={() => {setEditId(item.id); setDoorForm(item);}} className="text-blue-600 font-bold hover:underline">编辑</button><button onClick={() => handleDelete('materials_door', item.id)} className="text-gray-400 hover:text-rose-600">删除</button></td>
                    </tr>
                  ))}
                  {doors.length === 0 && <tr><td colSpan="4" className="p-8 text-center text-gray-400">暂无数据，请在上方添加</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        {currentView === 'upgrade' && (
          <div className="max-w-7xl mx-auto space-y-6 animate-fade-in">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
              ...
              <form onSubmit={handleSaveUpgrade} className="space-y-6">
                ...
                <div className="grid grid-cols-4 gap-6">
                  ...
                  <div><label className="block text-xs font-bold text-gray-500 mb-2">显示排序 (数字越小越前)</label><input type="number" value={upgradeForm.sort_order} onChange={e=>setUpgradeForm({...upgradeForm, sort_order:e.target.value})} className="w-full border-2 border-gray-200 p-3 rounded-xl focus:border-black focus:outline-none text-center" /></div>
                </div>
                
                {/* 第二排：计价参数 */}
                <div className="grid grid-cols-4 gap-6 items-end p-6 bg-blue-50/50 rounded-2xl border border-blue-100">
                  <div>
                    <label className="block text-xs font-bold text-blue-800 mb-2">系统计价方式 (数量抓取)</label>
                  ...
                  <div className="flex gap-2">
                    <div className="w-1/3"><label className="block text-xs font-bold text-gray-500 mb-2">单位</label><input value={upgradeForm.unit} onChange={e=>setUpgradeForm({...upgradeForm, unit:e.target.value})} className="w-full border-2 border-gray-200 p-3 rounded-xl text-center focus:border-black focus:outline-none" /></div>
                    ...
                    <div className="flex gap-2">
                      ...
                      {upgradeForm.upgrade_effect_type === 'replace' && (
                        <select value={upgradeForm.replace_calculation_mode} onChange={e=>setUpgradeForm({...upgradeForm, replace_calculation_mode:e.target.value})} className="flex-1 border-2 border-rose-200 bg-rose-50 p-3 rounded-xl text-rose-700 font-bold focus:outline-none text-xs">
                          <option value="full_price">填全价 (系统自动扣基础门板钱)</option>
                ...
                <div className="grid grid-cols-3 gap-6">
                  <div className="col-span-2"><label className="block text-xs font-bold text-gray-500 mb-2">后台管理备注说明 (指导销售填写)</label><input value={upgradeForm.description} onChange={e=>setUpgradeForm({...upgradeForm, description:e.target.value})} className="w-full border-2 border-gray-100 bg-gray-50 p-3 rounded-xl focus:bg-white focus:border-gray-300 focus:outline-none text-sm text-gray-600" placeholder="例如：此项需要人工核对图纸圆弧半径..." /></div>
                  <div className="flex flex-col gap-4 justify-center pt-6 px-4 bg-gray-50 rounded-xl border border-gray-100">
                    <label className="flex items-center gap-3 text-sm font-bold text-gray-700 cursor-pointer"><input type="checkbox" checked={upgradeForm.is_standard_item} onChange={e=>setUpgradeForm({...upgradeForm, is_standard_item:e.target.checked})} className="w-5 h-5 accent-black rounded" /> 设为开单默认标配勾选项</label>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 text-gray-500 font-bold border-b border-gray-200 text-xs uppercase tracking-wider"><tr className="divide-x divide-gray-100"><th className="p-4 w-16 text-center">排序</th><th className="p-4">项目名称</th><th className="p-4">分类</th><th className="p-4">计价方式 & 单价</th><th className="p-4">价格影响逻辑</th><th className="p-4 text-center">状态</th><th className="p-4 text-center">操作</th></tr></thead>
                <tbody className="divide-y divide-gray-100">
                  {upgrades.map(item => (
                    <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${!item.status ? 'opacity-50 grayscale' : ''}`}>
                      <td className="p-4 text-center font-medium text-gray-400">{item.sort_order}</td>
                      <td className="p-4"><div className="font-bold text-gray-900 text-base flex items-center gap-2">{item.name} {item.is_standard_item && <span className="text-[10px] bg-black text-white px-1.5 py-0.5 rounded uppercase">标配</span>}</div><div className="text-xs text-gray-400 mt-1 truncate max-w-[200px]">{item.description || '无备注'}</div></td>
                      <td className="p-4 font-bold text-gray-600">{item.upgrade_category}</td>
                      <td className="p-4"><div className="font-bold text-blue-700 mb-1">{item.calculation_type}</div><div className="text-sm font-medium">{item.calculation_type === '人工直接输金额' ? '按实输' : `¥${item.unit_price} / ${item.unit}`}</div></td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-center">{item.status ? <span className="w-3 h-3 bg-emerald-500 rounded-full inline-block shadow" title="正常使用"></span> : <span className="text-xs font-bold text-rose-500 border border-rose-500 px-2 py-1 rounded">已停用</span>}</td>
                      <td className="p-4 text-center space-x-3"><button onClick={() => {setEditId(item.id); setUpgradeForm(item);}} className="text-blue-600 font-bold hover:underline">编辑</button><button onClick={() => handleToggleUpgradeStatus(item)} className={`font-bold hover:underline ${item.status ? 'text-rose-500' : 'text-emerald-600'}`}>{item.status ? '停用' : '启用'}</button></td>
                    </tr>
                  ))}
                  {upgrades.length === 0 && <tr><td colSpan="7" className="p-8 text-center text-gray-400 font-medium">工艺库尚无数据，请在上方录入您的专属配置</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        {currentView === 'rules' && (
          <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
            ...
            <form onSubmit={handleSaveRules} className="bg-white p-10 rounded-2xl shadow-sm border border-gray-200 space-y-10 relative overflow-hidden">
              <div className="absolute top-0 right-0 bg-rose-50 text-rose-600 font-bold text-xs px-4 py-2 rounded-bl-2xl">⚠️ 核心参数，请谨慎修改</div>
              
              <div className="border-l-4 border-blue-500 pl-6">
                <h3 className="font-bold text-gray-900 mb-6 text-lg tracking-wide">📏 深度计价触发规则 (单位: mm)</h3>
                <div className="grid grid-cols-2 gap-8">
                  <div className="bg-gray-50 p-6 rounded-xl border border-gray-100"><label className="block text-sm font-bold text-gray-700 mb-3">标准深度界限 (Standard)</label><input type="number" required value={rules.standard_depth} onChange={e=>setRules({...rules, standard_depth:e.target.value})} className="w-full border-2 border-gray-200 p-4 rounded-xl text-xl font-black text-gray-900 focus:border-black focus:outline-none" />
                    <p className="text-xs text-gray-500 mt-3 leading-relaxed">系统逻辑：柜深超过此值，<span className="font-bold text-rose-600">柜体部分价格</span>将自动乘以 (实际深度 ÷ 标准深度) 进行超额加收。</p>
                  </div>
                  <div className="bg-gray-50 p-6 rounded-xl border border-gray-100"><label className="block text-sm font-bold text-gray-700 mb-3">浅柜判定界限 (Shallow)</label><input type="number" required value={rules.shallow_depth} onChange={e=>setRules({...rules, shallow_depth:e.target.value})} className="w-full border-2 border-gray-200 p-4 rounded-xl text-xl font-black text-gray-900 focus:border-black focus:outline-none" />
                    <p className="text-xs text-gray-500 mt-3 leading-relaxed">系统逻辑：柜深小于或等于此值时，直接调用对应材料库中的<span className="font-bold text-blue-600">“浅柜单价”</span>。</p>
                  </div>
                </div>
              </div>

              <div className="border-l-4 border-amber-500 pl-6 pt-6 border-t border-gray-100">
                <h3 className="font-bold text-gray-900 mb-6 text-lg tracking-wide">📐 计价模式判定与最低消费规则</h3>
                <div className="grid grid-cols-3 gap-6">
                  <div className="bg-gray-50 p-5 rounded-xl border border-gray-100"><label className="block text-sm font-bold text-gray-700 mb-3">高度计价分水岭 (mm)</label><input type="number" required value={rules.height_threshold} onChange={e=>setRules({...rules, height_threshold:e.target.value})} className="w-full border-2 border-gray-200 p-3 rounded-xl text-lg font-black text-gray-900 focus:border-black focus:outline-none" />
                    <p className="text-xs text-gray-500 mt-3 leading-relaxed">≤ 此高度，系统强行按【宽度的延米】计价。<br/>> 此高度，系统按【宽高投影面积】计价。</p>
                  </div>
                  <div className="bg-gray-50 p-5 rounded-xl border border-gray-100"><label className="block text-sm font-bold text-gray-700 mb-3">最低计算面积 (㎡)</label><input type="number" step="0.1" required value={rules.minimum_area} onChange={e=>setRules({...rules, minimum_area:e.target.value})} className="w-full border-2 border-gray-200 p-3 rounded-xl text-lg font-black text-gray-900 focus:border-black focus:outline-none" />
                    <p className="text-xs text-gray-500 mt-3 leading-relaxed">按面积算时，不足此值强行按此值计算 (保底消费)。</p>
                  </div>
                  <div className="bg-gray-50 p-5 rounded-xl border border-gray-100"><label className="block text-sm font-bold text-gray-700 mb-3">最低计算宽度 (mm)</label><input type="number" required value={rules.minimum_width} onChange={e=>setRules({...rules, minimum_width:e.target.value})} className="w-full border-2 border-gray-200 p-3 rounded-xl text-lg font-black text-gray-900 focus:border-black focus:outline-none" />
                    <p className="text-xs text-gray-500 mt-3 leading-relaxed">按延米算时，柜宽不足此值强行按此值计算。</p>
                  </div>
                </div>
              </div>

              <div className="pt-8 border-t border-gray-100"><button type="submit" className="w-full bg-black text-white p-5 rounded-2xl font-black shadow-xl hover:bg-gray-800 hover:shadow-2xl transition-all active:scale-[0.99] text-lg tracking-widest">保存全局引擎规则</button></div>
            </form>
          </div>
        )}
